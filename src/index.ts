...
const isValidExecutePythonFileArgs = (args: any): args is ExecutePythonFileArgs => {
  return (
    typeof args === 'object' &&
    args !== null &&
...
`;
  ...
  private async handleExecutePythonFile(args: unknown): Promise<{ content: { type: string; text: string; mediaType: string; }[]; isError?: boolean; }> {
    try {
      // Validate input arguments
      if (!isValidExecutePythonFileArgs(args)) {
        throw new ExecutorError(
          ErrorCode.INVALID_INPUT,
          'Missing or invalid filePath parameter.'
        );
      }
      ...
      try {
        ...
        await fs.access(filePath, fs.constants.F_OK)
          .catch(() => {
            throw new ExecutorError(
              ErrorCode.INVALID_INPUT,
              `Python file not found: ${filePath}`
            );
          });
