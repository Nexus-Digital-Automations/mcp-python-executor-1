...
export class VenvManager {
    ...
    validateVenvName(venvName) {
        ...
        if (venvName.length < 1 || venvName.length > 64) {
            throw new ExecutorError(ErrorCode.INVALID_INPUT, 'Virtual environment name must be between 1 and 64 characters');
        }
        // Only allow alphanumeric characters, hyphen, underscore
        const validNameRegex = /^[a-zA-Z0-9_-]+$/;
        if (!validNameRegex.test(venvName)) {
            throw new ExecutorError(ErrorCode.INVALID_INPUT, 'Virtual environment name can only contain letters, numbers, underscore and hyphen');
        }
        ...
        if (disallowedNames.includes(venvName.toLowerCase())) {
            throw new ExecutorError(ErrorCode.INVALID_INPUT, `"${venvName}" is not allowed as a virtual environment name`);
        }
    }
    /**
     ...
     */
    async withVenvLock(venvName, fn) {
     ...
     */
    async setupVirtualEnvironment(venvName, pythonVersion) {
        const targetVenvName = venvName || this.config.python.defaultVenvName;
        return this.withVenvLock(targetVenvName, async () => {
            ...
            if (!venvExists) {
                ...
                const pythonCmd = [pythonExecutableCommand, '-m', 'venv', '--clear', '--without-pip', venvPath];
                ...
                try {
                    ...
                    throw new ExecutorError(ErrorCode.INTERNAL_ERROR, `Failed to create or initialize virtual environment: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            else {
    ...
    async execCommand(command, options = {}) {
        ...
        if (Array.isArray(command)) {
            ...
            try {
                ...
                if (error.stdout)
                    this.logger.debug('Command stdout before error', { stdout: error.stdout.toString() });
                if (error.stderr)
                    this.logger.debug('Command stderr on error', { stderr: error.stderr.toString() });
        ...
        for (const name of venvNames) {
            try {
                ...
                try {
                    ...
                    const sitePkgPath = path.join(venvPath, os.platform() === 'win32' ? 'Lib\\site-packages' : 'lib/python*/site-packages');
                    ...
                    if (os.platform() !== 'win32') {
                        ...
                        if (pythonDir) {
                            ...
                            packageCount = files.filter(f => (f.endsWith('.dist-info') || f.endsWith('.egg-info') || !f.includes('.'))
                                && !['__pycache__', '.DS_Store'].includes(f)).length;
                        }
                    }
                    else {
     ...
     */
    async setVenvDescription(venvName, description) {
        // Validate the venv name
        this.validateVenvName(venvName);
        // Check if the venv exists
        if (!(await this.checkVenvExists(venvName))) {
            throw new ExecutorError(ErrorCode.INVALID_INPUT, `Virtual environment "${venvName}" does not exist`);
        }
     ...
     */
    async deleteVenv(venvName, force = false) {
        // Validate the venv name
        this.validateVenvName(venvName);
        // Don't allow deletion of the default environment unless forced
        if (venvName === this.config.python.defaultVenvName && !force) {
            throw new ExecutorError(ErrorCode.INVALID_INPUT, `Cannot delete the default environment "${venvName}" without force=true`);
        }
