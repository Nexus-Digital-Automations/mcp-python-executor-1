        ...
        try {
            ...
            try {
                ...
                if (this.config.python.analysis.enableComplexity && args.enableComplexity !== false) {
                    const { stdout: complexityOutput } = await execAsync(`radon cc ${scriptPath} -j --min ${this.config.python.analysis.maxComplexity}`);
                ...
                // Clean up temporary file
                await fs.unlink(scriptPath).catch(err => this.logger.error('Failed to clean up analysis file', { error: err.message }));
            }
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
