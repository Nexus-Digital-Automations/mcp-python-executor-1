...
export class VenvManager {
  ...
  private async execCommand(command: string, options: any = {}): Promise<{ stdout: string, stderr: string }> {
    ...
    try {
      ...
      if (error.stdout) this.logger.debug('Command stdout before error', { stdout: error.stdout });
      if (error.stderr) this.logger.debug('Command stderr on error', { stderr: error.stderr });
