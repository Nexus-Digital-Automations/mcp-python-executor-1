...
class PythonExecutorServer {
  ...
  public logger!: Logger; // Make logger definite assignment assertion
  private activeExecutions = 0;
  private cleanupInterval!: NodeJS.Timeout; // Make cleanupInterval definite assignment assertion
...
`;
  ...
  async run() {
    try {
      ...
      console.error(error instanceof Error && error.stack ? error.stack : 'No stack available');
  ...
  private async handleAnalyzeCode(args: any): Promise<{ content: { type: string; text: string; mediaType: string; }[]; isError?: boolean; }> {
    ...
    try {
      ...
      this.logger?.info?.('Using virtual environment for code analysis', { venvName: targetVenvName });
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger?.error?.('Code analysis error', { error: errorMessage, venvName: targetVenvName });
...
try {
  ...
  server.run().catch((error) => {
    ...
    console.error(`FATAL RUNTIME ERROR: ${error instanceof Error ? error.message : String(error)}`);
    console.error(error instanceof Error && error.stack ? error.stack : 'No stack trace available');
  ...
  console.error("STARTUP: After server.run() call (server running in background)"); // Log after calling async run

} catch (initError) {
  // Logger is definitely not initialized here, use console.error
