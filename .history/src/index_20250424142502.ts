...
`;
  ...
  private async handleExecutePython(args: unknown): Promise<{ content: { type: string; text: string; mediaType: string; }[]; isError?: boolean; }> {
    try {
      // Validate input arguments (Keep this outside the lock)
      if (!isValidExecuteArgs(args)) {
        throw new ExecutorError(
          ErrorCode.INVALID_INPUT,
          'Missing or invalid parameters. Please provide either code or scriptPath.'
        );
      }

      // Extract venv name if provided
      const venvName = (args as ExecutePythonArgs).venvName;
      const targetVenvName = venvName || this.config.python.defaultVenvName; // Determine the target name

      // --- Wrap the rest of the logic with the venv lock ---
      return await this.venvManager.withVenvLock(targetVenvName, async () => {
        // Verify the environment exists or create it (this is now called inside the lock)
        ...
        try {
          ...
          // Use the validated targetVenvName
          ...
          if ((args as ExecutePythonArgs).code) {
            ...
            await fs.writeFile(scriptPath, (args as ExecutePythonArgs).code!, 'utf-8');
            tempFile = true;
          } else if ((args as ExecutePythonArgs).scriptPath) {
            // Use the provided script path
            scriptPath = (args as ExecutePythonArgs).scriptPath!;
          ...
          let inputData = '';
          if ((args as ExecutePythonArgs).inputData && Array.isArray((args as ExecutePythonArgs).inputData)) {
            inputData = (args as ExecutePythonArgs).inputData!.join('\n') + '\n';
          }
          ...
          try {
            if (isWindows) {
              ...
              // Using venvManager.execCommand with string command (legacy mode)
              const result = await this.venvManager.execCommand(command, execOptions);
              stdout = result.stdout;
              stderr = result.stderr;
            } else {
              // On Unix, we can use spawn directly with the Python executable through venvManager.execCommand
              const execOptions = {
                timeout: this.config.execution.timeoutMs,
                env: { ...process.env },
                input: inputData || undefined
              ...
              const result = await this.venvManager.execCommand([pythonExecutable, scriptPath], execOptions);
              stdout = result.stdout;
              stderr = result.stderr;
            }
          } finally {
      ...
      }); // End of withVenvLock wrapper
    } catch (error: any) { // Catch any errors thrown outside or inside the lock
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error('Python execution error', { error: errorMessage, venvName: (args as any)?.venvName });
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              ...
              venvName: (args as any)?.venvName || this.config.python.defaultVenvName // Use venvName from args or default
            }, null, 2),
            mediaType: 'application/json'
          }
        ],
